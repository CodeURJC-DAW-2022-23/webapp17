...
public interface UsersRepo extends JpaRepository<UserProfile, Long>{
    ...
    @Query(value="select product_id from (select created_by_id, product_id from cart join cart_item on cart_item.cart_id=cart.id) as t "+
    "join (select created_by_id as c, product_id as p from cart join cart_item on cart_item.cart_id=cart.id) as q where "+
